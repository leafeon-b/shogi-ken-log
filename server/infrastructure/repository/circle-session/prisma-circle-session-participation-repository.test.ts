...
describe("Prisma CircleSession 参加者リポジトリ", () => {
  ...
  test("areParticipants は全員参加なら true", async () => {
    ...
    const result = await prismaCircleSessionParticipationRepository.areParticipants(
  ...
  test("listParticipants は参加者一覧を返す", async () => {
    ...
    const result = await prismaCircleSessionParticipationRepository.listParticipants(
      circleSessionId("session-1"),
    );
  ...
  test("removeParticipant は参加者を削除する", async () => {
    ...
    expect(mockedPrisma.circleSessionMembership.deleteMany).toHaveBeenCalledWith(
      {
        where: {
          circleSessionId: "session-1",
          userId: "user-1",
